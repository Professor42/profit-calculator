...
async def calculate_fee(ctx, price):
    try:
        ...
        # Create clean text output like the Moonitor bot
        ...
        for platform in platforms:
            ...
            if result:
                fee_list = []
                for fee_name, fee_amount in result['fees'].items():
                    fee_list.append(f"{fee_name}: ${fee_amount:.2f}")
                
                fees_text = " + ".join(fee_list)
                output += f"**{result['platform']}** (${result['total_fees']:.2f})\n${result['net_payout']:.2f}\n\n"
...
async def help_command(ctx):
    ...
    embed.add_field(
        name="Commands",
        value="`$fee 350` - Calculate fees\n`$compare 350` - Compare platforms\n`$profit 200 350 ebay` - Profit analysis",
        inline=False
    )
